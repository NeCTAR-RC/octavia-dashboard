...
class AvailabilityZones(generic.View):
    ...
    def get(self, request):
        ...
        """
        conn = _get_sdk_connection(request)
        availabilityzone_list = _sdk_object_to_list(
            conn.load_balancer.availabilityzones()
        )

        return {'items': availabilityzone_list}

    @rest_utils.ajax()
    def post(self, request):
        """Create a new availabilityzone.

        """
        kwargs = {
            'availabilityzone': request.DATA.get('availabilityzone')
        }
        return create_availabilityzone(request, **kwargs)
